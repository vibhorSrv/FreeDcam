...
#ifdef tracemethods
	Expr dist = print(sum(abs(i32(ref_val) - i32(alt_val))) / 256, "distance tx:", tx, " ty:", ty);

	Expr norm_dist = print(max(1, i32(dist) - min_dist), "norm distance tx:", tx, " ty:", ty);
#endif // !tracemethods
#ifndef tracemethods
...
/*#ifdef tracestores
	total_weight.trace_stores();
	total_weight.trace_loads();
#endif*/
 ...
 * dimension to produce one denoised bayer frame.
 */
Func merge(Halide::ImageParam imgs, Halide::ImageParam alignment, Expr minoffset, Expr maxoffset, Expr l1mindist, Expr l1maxdist) {
	
    Func merge_temporal_output = merge_temporal(imgs, Func(alignment), minoffset, maxoffset, l1mindist,l1maxdist);
